...
InterfazGrafConecta4::InterfazGrafConecta4(int w, int h) : InterfazGrafT2(w,h), ladoCasilla(window_width/(8+2)) {}
InterfazGrafConecta4::~InterfazGrafConecta4(){}

void InterfazGrafConecta4::muestraEst(Graphics^ canvas, const JuegoConecta4& EJ) const throw(){
...
void  InterfazGrafConecta4::muestraEstado(Graphics^ canvas, const JuegoLogT2& EJ)const throw(EInterfaz) { // posible error de casting
	if (typeid(EJ) != typeid(JuegoConecta4))
		throw EInterfaz();
	else {
		muestraEst(canvas, (const JuegoConecta4&) EJ);
	}
};
